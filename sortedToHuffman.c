...
void printCodes(struct MinHeapNode* root, nodeBST **arr, int top) 
...
void HuffmanCodes(nodeBST **words, int size) 
  
{ 
    // Construct Huffman Tree 
    struct MinHeapNode* root 
        = buildHuffmanTree(words, size); 
  
    // Print Huffman codes using 
    // the Huffman tree built above 
    int arr[MAX_TREE_HT], top = 0; 
  
    printCodes(root, words, top); 
} 
